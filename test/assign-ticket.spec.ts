...
describe('Assign Ticket Use Case', () => {
  beforeEach(() => {
    ...
    sut = new AssignTicketUseCase(inMemoryTicketRepository, inMemoryTechnicianRepository, assignmentService)
  })
  ...
  it('should be able assign a ticket', async () => {
    ...
    const ticket = makeTicket({ title: 'Support needed', description: 'Help me please' }, new UniqueEntityId('ticket-1'))
    ...
    if (result.isRight()) {
      expect(inMemoryTechnicianRepository.items.get(technician.id.toString())?.ticketsAssigned).toHaveLength(1)
      expect(inMemoryTicketRepository.items.get(ticket.id.toString())?.assignedBy?.toString()).toEqual(technician.id.toString())
      expect(inMemoryTicketRepository.items.get(ticket.id.toString())?.status.value).toEqual('ASSIGNED')
    }
  })
  ...
  it('should not be able to assign a ticket if technician reached max concurrent tickets', async () => {
    ...
    const ticket = makeTicket({ title: 'Support needed', description: 'Help me please' }, new UniqueEntityId('ticket-2'))
    await inMemoryTicketRepository.create(ticket)
