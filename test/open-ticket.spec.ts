...
describe('Open Ticket Use Case', () => {
  beforeEach(() => {
    ...
    sut = new OpenTicketUseCase(inMemoryTicketRepository, inMemoryClientRepository)
  })
  ...
  it('should be able open a ticket', async () => {
    ...
    if (result.isRight()) {
      ...
      expect(inMemoryTicketRepository.items.has(result.value.ticket.id.toString())).toBe(true)
